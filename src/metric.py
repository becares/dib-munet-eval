...
def metrics(im_path, im_gt_path):
...
if __name__ == "__main__":
	...
	predictions_path = '../inference/image'
	labels_path = '../dataset/testing/label'

	predictions = subfiles(predictions_path, join=False)
	labels = subfiles(labels_path, join=False)
	
	assert len(predictions) == len(labels)
    
	df = pd.DataFrame(columns=['img','fmeasure','pfmeasure','psnr','nrm','mpm','drd'])
	
	for i in range(len(predictions)):
		img = predictions[i].split('.')[0]
		pred_path = join(predictions_path, predictions[i])
		label_path = join(labels_path, labels[i])

		fmeasure, pfmeasure, psnr, nrm, mpm, drd = metrics(pred_path, label_path)
	...
	df.to_csv('../inference/results.csv')
